...
function Catalogo() {
  ...
  return (
    <div className="container mt-5">
      ...
      {/* --- BOTONES DE NAVEGACIÓN (usando d-flex para responsividad) --- */}
      {/* Se asegura el espaciado correcto con mx-2, mb-4 y la alineación a la derecha en pantallas grandes */}
      <div className="d-flex justify-content-center justify-content-md-end mb-4">
        <Link to="/login" className="btn mx-2" style={linkStyle}>
          Iniciar sesión
        </Link>
        <Link to="/carrito" className="btn" style={linkStyle}>
          Carrito de compras
        </Link>
      </div>
      {/* --- FIN DE BOTONES --- */}
        ...
        <div className="productos">
          {productosFiltrados.map((producto, index) => (
            <div key={index} className="producto" onClick={() => handleVerProducto(producto)} style={{ cursor: 'pointer' }}>
              ...
              <button className="btn btn-dark w-100" onClick={() => agregarAlCarrito(producto.nombre)}>
